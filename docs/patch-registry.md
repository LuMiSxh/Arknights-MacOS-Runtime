...
| `wine-cn-relative-wait`       | `patches/wine/cn/timing/0001-ntdll-cn-qpc-relative-wait.patch`       | Negative relative-wait route                    |
...
so both inactive and enabled execution paths require validation. Removal occurs when accepted
Wine/WineCX behavior replaces the port or a compatibility route is no longer required. This family is
compiled into the combined artifact and remains inactive by default.
